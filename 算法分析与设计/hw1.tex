...
\begin{solution}
    ...
    a. 正确。因为 $t(n) \in O(g(n))$，所以存在 $c > 0$，$n_0 > 0$，使得当 $ n \ge n_0$ 时，满足 $t(n) \leq c \cdot g(n)$。\ 
    不等式两边同时除以常数c得 $\frac{1}{c} \cdot t(n) \leq g(n)$，即 $k \cdot t(n) \leq g(n)$，其中 $k > 0$，所以 \
    $g(n) \in \Omega(t(n))$。
    \vspace{2mm}  % 指定高度，添加空行
    
    b. 正确。 $\Theta(\alpha g(n)) = \{f(n)$:存在 $c_1 > 0$, $c_2 > 0$， $n_1 > 0$，使得当 $n \ge n_1$ 时，满足 $c_2 \alpha g(n) \le f(n) \
    \le c_1 \alpha g(n)$。同理， $\Theta(g(n)) = \{f(n)$:存在 $d_1 >0$，$d_2 >0$，$n_2 > 0$，使得当 $n \ge n_2$ 时，满足 $d_2 g(n) \le f(n) \
    \le d_1 g(n)$，因为 $c_1 > 0$，$c_2 > 0$，$d_1 >0$，$d_2 >0$，且 $\alpha > 0$，所以 $d_2$、$d_1$总可以取到与 $\alpha c_2$、$\alpha c_1$相等的值，\
    因此， $\Theta(\alpha g(n))$ 和 $\Theta(g(n))$ 表示的是同一个集合，故 $\Theta(\alpha g(n)) = \Theta(g(n))$。
    \vspace{2mm}  % 指定高度，添加空行

    c. 正确。 $\Theta(g(n)) = \{f(n)$:存在 $c_1 >0$，$c_2 >0$，$n_0 > 0$，使得当 $n \ge n_0$ 时，满足 $c_2 g(n) \le f(n) \
    \le c_1 g(n)$。 $O(g(n)) = \{f(n)$:存在 $d_1 > 0$，$n_1 > 0$，使得当 $n > n_1$ 时，满足 $f(n) \le  d_1 g(n)$。 $\Omega(g(n)) = \{f(n)$:存在 $d_2 > 0$，$n_2 > 0$ \
    ，使得当 $n > d_2$ 时，满足 $f(n) \ge d_2 g(n)$。令 $c_2 = d_1$，$c_1 = d_2$，则这两个表示同一个集合，故 $\Theta(g(n)) = O(g(n)) \cap \Omega(g(n))$。
    \vspace{2mm}  % 指定高度，添加空行
...
\begin{solution}
    ...
    执行次数： $C(n) = C(n/2) + 1 ~(n \ge 1)$ 和 $C(0) = 0$

    根据平滑性原则，令 $n=2^k$，则有 $C(2^k) = C(2^{k-1}) + 1$，$C(2^k) = ··· = C(2^{k-k}) + k = k$，所以有 $C(2^n) = n$，故 $C(n) = \log_{2}(n)$
